...
https://releases.dataone.org/online/api-documentation-v2.0.0/design/SystemMetadata.html.
...
An optional *user* argument can be specified when creating a *DataObject*, which will be used to set the DataONE *submitter* and *rightsholder* of the dataset when it is uploaded. The *rightsholder* is the
DataONE user that has full priviledge to the object.

If a *user* is not specified for a *DataObject*, then it will automatically be set to the DataONE user 
that created the authentication token or X.509 certificate. 
...
The *updateObject* updates an existing object by creating a new object identified by a new PID on the Member Node which explicitly obsoletes the object identified by pid through appropriate changes to the SystemMetadata of PID and the new PID.
...
The Member Node will mark the object as being obsoleted by setting the system metadata obsoletedBy property on the object being  obsoleted to the PID of the new object. 

The updateObject operation will fail with on objects that have the system metadata archived property set to true.
...
referenced with it's PID and downloaded, but it will not appear in any search results.

Objects that are archived can not be updated using the *updatei* method.  Once an object is archived it cannot be un-archived. 

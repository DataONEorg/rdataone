...
# Ask for the id, title and abstract
queryParams <- list(q="id:doi*", fq="abstract:kelp", fl="id,title,abstract") 
result <- query(cn, solrQuery=queryParams, as="data.frame", parse=FALSE)
pid <- result[1,'id']
dataObj <- getDataObject(d1c, pid)
```

This method uses the *getDataObject* method from the R package *datapackage*.  The *getDataObject* method determines which 
member node in DataONE holds the data item and downloads it into a datapackage::DataObject. The DataObject R object is 
a wrapper that contains the data bytes for the DataONE dataset requested, as well as the and returns a DataObject which includes
the data itself and the DataONE system metadata for the object. The following functions are available to extract
...
To extract the data byes from the DataObject:
```{r, warnings=F}
dataBytes <- getData(dataObj)
```

Note that the complete set of information from the DataONE system metadata is available from the 
object's slot:

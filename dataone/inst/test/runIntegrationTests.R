...
testDir <- function() {
    ...
    if (grepl("d1_client_r",curDir)) {
        ...
        testDir <- sub("d1_client_r.+","d1_client_r/dataone/inst/test/integration",curDir)
    } else {
        testDir <- file.path(.path.package(package="dataone"),"inst/test/integration")
    }
    return(testDir)
}

theTestSuite <- defineTestSuite("integrationTests",
        dirs = testDir(),

...
A *DataPackge* is a `dataone` R object that can contain a DataONE package that is either downloaded from
DataONE. Also, a *DataPackage* can be created locally in R and then uploaded to DataONE.
...
file `filteredSpecies.csv.zip` instead of `filteredSpecies.csv`. After this change, the DataObject
...
metadata document. The EML elewent `objectName` is updated with the value `filteredSpecies.csv.zip`.
...
In this example, the section of the metadata that describes the dataset `filteredSpecies.csv.zip` will be 
updated. 

A portion of the EML from our exmaple looks like this:
```
    <otherEntity>
      <entityName>filteredSpecies</entityName>
      <entityDescription>A data file that was filtered from the input date to contain only the species MUSS</entityDescription>
      <physical>
        <objectName>filteredSpecies.csv</objectName>
...
value. This package has only one *DataObject* with the name `filteredSpecies.csv.zip` so only

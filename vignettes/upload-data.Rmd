...
After a package has been uploaded to DataONE, it may be determined by the package submitter that the package needs to be 
updated, for example to add a missing file, replace one file with another, or remove a package member from the package.

These types of modifications can be accomplished by downloading the package from DataONE using the `getDataPackage` method so
that a local copy can be modified in R, then updated back to the DataONE Member Node.
...
First, a sample EML metadata file will be added to a DataPackage. This EML file contains descriptions
of all the DataObjects that will be uploaded in this example package.
...
The next step shows how to update the EML DataObject that is contained within the DataPackage `dp`.

The metadata file has a section that describes the dataset `sample.csv` (an EML `<otherEntity>` element), which contains the 
EML element `<physical><distribution><online><url>`. This EML element contains the link that can be used to download the
file from DataONE, which includes the DataONE identifier for the object. This link value can be updated with 
the identifier value that was just assigned to the DataObject, that will become the DataONE identifier when the 
package is uploaded.
...
The `updateMetadata` call updates the DataObject specified with the `do` argument. This argument can also 
...
Also, `updateMedata` will update the identifier associated with the modified DataObject if necessary,
and retain the original identifier. The original identifer (used for the initially uploaded object) and a new identifier 
are required by  DataONE when the modified object is uploaded to DataONE. If the `newId` parameter 
is not included in the call to `updateMetadata`, then the new identifier will be generated automatically.
Alternatively, the `newId` parameter can be used to specify the value of the new identifier, which will also
force identifier to be updated even if it is not required. Typically a new identifier is generated
by `updateMetadata` only the first time this method is called for a particular DataObject.

The identifier for a DataObject that is a member of a DataPackage can be retrieved using the `selectMember` method:
```{r}
metadataId <- selectMember(dp, name="sysmeta@formatId", value="eml://ecoinformatics.org/eml-2.1.1")
```

In this example, `selectMember` checks the `sysmeta@formatId` slot of each DataObject in the DataPackage `dp`
and returns the identifier of any member that matches the specified value (`eml://ecoinformatics.org/eml-2.1.1` in this case).
Only the DataObject that contains the EML metadata matches for the example DataPackage, so the identifier for 
just that member is returned.
...
The DataPackage can be reviewed before uploading it:
...
this demonstration will use a saved object that was previously created by uploading a package, then downloading
it with the `getDataPackage` method ash shown below:
...
The next example shows how to update the example package that was uploaded to DataONE.
Modifications can be made to the package such as adding or removing members from the package, or
changing the contents of a package member, as would be the case if the wrong file was initially
uploaded.
...
the `identifier` parameter. Because packages might contain large files or a large number of files, it is
...
### Replace the contents of a package member
The original uploaded package included the file `filterSpecies.csv`. The zipped form of this
file should have been used instead, so the DataObject within the DataPackage will be updated to 
use the zipped file.

First, determine which DataObject in the DataPackage `pkg` contains the errant file:
```{r}
objId <- selectMember(pkg, name="sysmeta@fileName", value='filteredSpecies.csv')
```

Next, update the DataObject in `pkg` to replace the file `filteredSpecies.csv` with `filteredSpecies.csv.zip` 
...
objId <- selectMember(pkg, name="sysmeta@fileName", value='filteredSpecies.csv.zip')
```
...
to the `updateMetadata` method, if the `newId` parameter is not specified for `replaceMember`, then one will be automatically generated if necessary.

Next, update the filename in the EML for the relevant section:
```{r}
...
```
In this example, the file `collectionMethods.csv` was mistakenly omitted from the original package upload, although it is
described in the EML metadata. This file will be added to the DataPackage now:
...
Now upload the modified package to DataONE. Each DataObject in the DataPackage will be inspected by `uploadDataPackage` and
ones that have been modified will be updated. DataObjects that have not been modified will not be updated
to DataONE.
```{r, eval=FALSE}

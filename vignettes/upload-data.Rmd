...
token is described in the *dataone-overview* vignette, in the section **DataONE User Authentication**, which is viewable with the R command `vignette("dataone-overview")`. (Note: DataONE originally used X.509 certificates for authentication, which are still supported.)
...
[Ecological Metadata Langauage](https://knb.ecoinformatics.org/#external//emlparser/docs/index.html) (EML). Other supported formats include FGDC, ISO 19115 and others.
...
the relationship that links a metadata object and a science object is [CITO](http://vocab.ox.ac.uk/cito) *documents*. 
...
As the metadata object has already been added, it can be refernced as each DataObject is added.

```{r, eval=FALSE}
dp <- addMember(dp, sourceObj, metadataObj)
```

Since `metadataObj` is included as the third argument here, the [CITO](http://vocab.ox.ac.uk/cito) *documents*
relationship will automatically be added between `metadataObj` and `sourceObj`.

Alternatively, this relationship between the metadata and science objects can be made explictly using
...
Note that the relationship type, using the `insertRelationship()` `predicate` argument does not have to be specified in this case, as the [CITO](http://vocab.ox.ac.uk/cito) *documents* relationship is the default value for `insertRelationship`.
...
The access rules that are added to DataObjects in a DataPackage will determine the acces that is granted
...
user with the ORCID `http://orcid.org/0000-0002-2192-403X` `changePermission` access to the dataset:
...
After *uploadDataPackage* has been called sucessfully, the package can be viewed on the member node, searched for
...
A Digital Object Identfier (DOI) may be assigned to the metadata *DataObject*, using the *generateIdentifier* method:

...
user 'jsmith' changePermission access to the dataset after it is uploaded and available on a DataONE MN:
...
another object, etc. A system metadata object is created in R using the *datapack::SystemMetadata*
...
cn <- CNode("STAGING")
mn <- getMNode(cn, "urn:node:mnStageUCSB2")
examplePid <- "urn:uuid:17d61d5a-061a-4778-9cdf-4e14751aaddc"
sysmeta <- getSystemMetadata(mn, examplePid)
sysmeta <- addAccessRule(sysmeta, "public", "read")
status <- updateSystemMetadata(mn, examplePid, sysmeta)
```
